...
- Fixed all 4 P2 OWASP issues: 5. Added idempotencyKey (randomUUID) to Stripe checkout.sessions.create() 6. Added rate limit (10/min) to /api/checkout 7. Added structured logging in authorize() — console.warn on failure (user not found, invalid password), console.info on success; logs email + IP + userId (never password) 8. Added SSRF allowlist in downloadImage() — validates hostname against ['replicate.delivery', 'replicate.com'] before fetching
- Fixed all 3 P1 WCAG issues: 9. Raised --color-muted from #6a6a6a (3.66:1) to #8a8a8a (5.5:1) — passes AA-normal 10. Added global CSS rule for focus-visible on all interactive elements: a[href]:focus-visible, button:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible, [tabindex]:focus-visible { outline: 2px solid var(--color-accent); outline-offset: 2px; } 11. Bumped all icon buttons from h-10 w-10 (40px) to h-11 w-11 (44px); wrapped carousel dots in min-h-11 min-w-11 tap target containers
...
Task ID: AUDIT-v2 (external audit — no code changes)
Agent: Super Z (Frontend Architect & Avant-Garde UI Designer)
Task: Systematic Six-Axis code review + live-site E2E + documentation reconciliation audit.

Work Log:

- Read CLAUDE.md, AGENTS.md, status_3.md, README.md, skills/skills-catalog.md.
- Validated doc claims against code (proxy.ts ✅, StatBlock toLocaleString('en-US') ✅, IdSchema ✅, published filter ✅, no casts ✅, no @ts-expect-error ✅).
- Phase A: typecheck/lint/test/audit all green (183/183 tests, 0 vulns).
- Phase B: Six-Axis review of 95+ files. Found D1 (CSP unsafe-eval still present — Critical), M3 (Stripe webhook stub), M4 (Inngest stubbed), M5 (ratelimit imports env), L1-L9.
- Phase C: agent-browser live-site validation (8 journeys functional). Confirmed dev mode + localhost sitemap. Playwright SEO spec 12/13 pass.
- Phase D: 5 docs claim H1 CSP fix was applied — code contradicts all 5.
- Phase E: ~17% test coverage; missing regression test for D1.
- Phase F: Generated audit report (Markdown + 16-page PDF) at /home/z/my-project/download/IRONFORGE_audit_report_v2.{md,pdf}.

Stage Summary:

- 1 Critical + 2 High + 5 Medium + 9 Low/Info findings.
- NO code changes applied (audit only).
- See /home/z/my-project/download/IRONFORGE_audit_report_v2.pdf for the full report.

---

Task ID: REMEDIATE-v2 (code fixes applied via TDD)
Agent: Super Z (Frontend Architect & Avant-Garde UI Designer)
Task: Execute 6 code-fixable findings from audit v2 using TDD.

Work Log:

- F-S1 (High): Created .env.example with placeholders, untracked .env.local + .env.docker, deleted .env.docker.
- F-D1 (Critical, TDD): Removed 'unsafe-eval' from next.config.ts:30. Added csp-policy.test.ts (6 tests).
- F-M5 (Medium, TDD): Refactored ratelimit.ts to use process.env (removed @/lib/env import). Fixed hasRealRedis() to check 'xxx' in addition to 'placeholder'. Added ratelimit.test.ts (4 tests).
- F-S2 (High): Deleted playwright-live.config.ts. Removed test:e2e:live + audit:security + audit:a11y from package.json. Updated README.md.
- F-M3 (Medium, TDD): Implemented 3 Stripe webhook handlers (checkout.session.completed, customer.subscription.updated, customer.subscription.deleted). Removed `as unknown as` cast — Stripe SDK v22 uses snake_case (verified). Fixed file header comment. Added priceId to checkout metadata. Added stripe-webhook.test.ts (10 tests).
- F-M4 (Medium, TDD): `pnpm add resend`. Created src/lib/email/resend.ts (graceful-degradation client). Wired trial-requested.ts to send real emails via Resend (with console.log fallback). Added RESEND_FROM_EMAIL + COACH_NOTIFY_EMAIL to env.ts. Added trial-requested.test.ts (4 tests).
- Quality gate: typecheck ✅, lint ✅, test 207/207 ✅ (+24 new), build ✅.

Stage Summary:

- 6 findings remediated. 183 → 207 tests. 0 type errors. 0 lint errors. Clean build.
- Operational items (F-M1 deploy, F-M2 NEXT_PUBLIC_APP_URL, migration 0002, Stripe config, AUTH_SECRET rotation) require deployment env access — documented, not code-fixed.
